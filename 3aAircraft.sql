...
SELECT Aircraft_name FROM AIRCRAFT WHERE Aircraft_ID NOT IN (SELECT Aircraft_ID FROM CERTIFIED JOIN EMPLOYEE ON CERTIFIED.Emp_ID = EMPLOYEE.Emp_ID WHERE Salary <= 50000);


SELECT Ename FROM EMPLOYEE WHERE Emp_ID NOT IN (SELECT Emp_ID FROM CERTIFIED);